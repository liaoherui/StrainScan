...
* *(only GitHub version)StrainScan is able to take the custom clustering file generated by other clustering methods (e.g. PopPunk)! Also, we have updated a script (StrainScan_subsample.py) to allow users to subsample their strains with the hierarchical clustering. <BR/>*
